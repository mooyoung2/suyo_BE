...
MAX=dict(size=10,cagr=15,momentum=5,gender=5,age=5,demand=8,weekend=10,density=15,survival=15)
...
for key,mx in [('l1',30),('l2',28),('l3',30),('total',88)]:
...
for nm,gu in [('카페','중랑구'),('카페','강남구'),('카페','종로구'),('일반의원','중랑구'),('편의점','강남구')]:
    ...
    r=hit[0]
    print(f"{nm}/{gu} (점포 {r['n']}개): L1 {r['l1']:.1f}({gr('l1',r['l1'])}) L2 {r['l2']:.1f}({gr('l2',r['l2'])}) L3 {r['l3']:.1f}({gr('l3',r['l3'])}) → 총 {r['total']:.1f}/88 {gr('total',r['total'])}")
